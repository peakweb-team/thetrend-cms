...
/**
 * Directory Lifecycle Hooks - i18n Slug Synchronization
 *
 * Purpose: Keep slugs consistent across all locale versions of a directory entry.
 * Slugs are managed ONLY by the default locale (English) and automatically
 * synced to all other locales.
 *
 * Why: URLs should remain consistent regardless of language. A directory entry at
 * /en/directory/company-name should also exist at /es/directory/company-name.
 *
 * How it works:
...
module.exports = {
  ...
  async beforeCreate(event) {
    ...
    if (locale && locale !== DEFAULT_LOCALE && documentId) {
      const defaultEntry = await strapi.documents('api::directory.directory').findOne({
      ...
      setImmediate(async () => {
        try {
          ...
          for (const loc of otherLocales) {
            try {
              const localeEntry = await strapi.documents('api::directory.directory').findOne({
                documentId: documentId,
                locale: loc.code,
                fields: ['slug'],
              });
              ...
              if (localeEntry && localeEntry.slug !== newSlug) {
                await strapi.documents('api::directory.directory').update({
                  documentId: documentId,
                  locale: loc.code,
                  data: { slug: newSlug },
                });
