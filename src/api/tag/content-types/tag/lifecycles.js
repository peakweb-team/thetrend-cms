...
/**
 * Tag Lifecycle Hooks - i18n Slug Synchronization
 *
 * Purpose: Keep slugs consistent across all locale versions of a tag.
 * Slugs are managed ONLY by the default locale (English) and automatically
 * synced to all other locales.
 *
 * Why: URLs should remain consistent regardless of language. A tag at
 * /en/tags/javascript should also exist at /es/tags/javascript.
 *
 * How it works:
...
module.exports = {
  ...
  async beforeCreate(event) {
    ...
    if (locale && locale !== DEFAULT_LOCALE && documentId) {
      const defaultEntry = await strapi.documents('api::tag.tag').findOne({
      ...
      setImmediate(async () => {
        try {
          ...
          for (const loc of otherLocales) {
            try {
              const localeEntry = await strapi.documents('api::tag.tag').findOne({
                documentId: documentId,
                locale: loc.code,
                fields: ['slug'],
              });
