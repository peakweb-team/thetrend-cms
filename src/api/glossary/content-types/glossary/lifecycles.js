...
/**
 * Glossary Lifecycle Hooks - i18n Slug Synchronization
 *
 * Purpose: Keep slugs consistent across all locale versions of a glossary term.
 * Slugs are managed ONLY by the default locale (English) and automatically
 * synced to all other locales.
 *
 * Why: URLs should remain consistent regardless of language. A glossary term at
 * /en/glossary/api should also exist at /es/glossary/api.
 *
 * How it works:
...
module.exports = {
  ...
  async beforeCreate(event) {
    ...
    if (locale && locale !== DEFAULT_LOCALE && documentId) {
      const defaultEntry = await strapi.documents('api::glossary.glossary').findOne({
      ...
      setImmediate(async () => {
        try {
          ...
          for (const loc of otherLocales) {
            try {
              const localeEntry = await strapi.documents('api::glossary.glossary').findOne({
                documentId: documentId,
                locale: loc.code,
                fields: ['slug'],
              });
              ...
              if (localeEntry && localeEntry.slug !== newSlug) {
                await strapi.documents('api::glossary.glossary').update({
                  documentId: documentId,
                  locale: loc.code,
                  data: { slug: newSlug },
                });
