...
/**
 * Category Lifecycle Hooks - i18n Slug Synchronization
 *
 * Purpose: Keep slugs consistent across all locale versions of a category.
 ...
 * /en/categories/tech should also exist at /es/categories/tech.
 *
 * How it works:
...
module.exports = {
  ...
  async beforeCreate(event) {
    ...
    if (locale && locale !== DEFAULT_LOCALE && documentId) {
      const defaultEntry = await strapi.documents('api::category.category').findOne({
      ...
      setImmediate(async () => {
        try {
          ...
          for (const loc of otherLocales) {
            try {
              const localeEntry = await strapi.documents('api::category.category').findOne({
                documentId: documentId,
                locale: loc.code,
                fields: ['slug'],
              });
