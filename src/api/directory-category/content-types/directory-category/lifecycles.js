...
/**
 * Directory Category Lifecycle Hooks - i18n Slug Synchronization
 *
 * Purpose: Keep slugs consistent across all locale versions of a directory category.
 ...
 * /en/directory/categories/software should also exist at /es/directory/categories/software.
 *
 * How it works:
...
module.exports = {
  ...
  async beforeCreate(event) {
    ...
    if (locale && locale !== DEFAULT_LOCALE && documentId) {
      const defaultEntry = await strapi.documents('api::directory-category.directory-category').findOne({
      ...
      setImmediate(async () => {
        try {
          ...
          for (const loc of otherLocales) {
            try {
              const localeEntry = await strapi.documents('api::directory-category.directory-category').findOne({
                documentId: documentId,
                locale: loc.code,
                fields: ['slug'],
              });
              ...
              if (localeEntry && localeEntry.slug !== newSlug) {
                await strapi.documents('api::directory-category.directory-category').update({
                  documentId: documentId,
                  locale: loc.code,
                  data: { slug: newSlug },
                });
