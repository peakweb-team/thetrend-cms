...
| `metaDescription` | string    | No       | Yes       | 160        | Page description for search results |
| `metaImage`       | media     | No       | No        | -          | Default image for social sharing    |
| `keywords`        | string    | No       | Yes       | 255        | Comma-separated keywords            |
| `canonicalURL`    | string    | No       | Yes       | -          | Canonical URL override (see below)  |
...
-   **URL Format**: Must include the full protocol (e.g., `https://thetrend.tech/articles/my-article`)
-   URLs without protocol (e.g., `/articles/my-article`) will be treated as relative paths and appended to the base URL
